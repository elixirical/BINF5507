...
def impute_missing_values(data, strategy='mean'):
    ...
    # iterates through every column name in data
...
def normalize_data(data,method='minmax'):
    ...
    numeric_feats = data.select_dtypes('number')

    # creates a new df, with normalized numbers
    scaled_data = pd.DataFrame(scaler.fit_transform(numeric_feats))
    scaled_data.columns = numeric_feats.columns

    # ensures all numeric columns are of type float64 in original data 
    for col in data.columns:
